...
class EstudiantesController{
    ...
    static async indexPost(req,res){
        try {
            ...
            const insertedId = await estudiantesModel.insertar(newData);

            res.status(201)
                .header('Location', `/estudiantes/${insertedId}`)
    ...
    static async itemPut(req, res) {
        try {
            ...
            const result = await estudiantesModel.reemplazar(id, updatedData);
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await estudiantesModel.actualizar(id, updatedFields);
