...
class respuestasTestController{
    ...
    static async indexPost(req,res){
        try {
            ...
            const insertedId = await RespuestasTestModel.insertar(newData);

            res.status(201)
                .header('Location', `/resp_test/${insertedId}`)
    ...
    static async itemPut(req, res) {
        try {
            ...
            const result = await RespuestasTestModel.reemplazar(id, updatedData);
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await RespuestasTestModel.actualizar(id, updatedFields);
